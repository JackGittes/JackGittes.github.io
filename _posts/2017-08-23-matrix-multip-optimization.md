...
　　单行分割即每一行作为一块，和向量进行乘加得到一个结果，这是最普通常见的方式。单列分割是将矩阵$A_{mn}$分成$n$列，第$i$列的所有元素和第$i$个向量元相乘，这样得到$n$个相乘之后的列向量，把这些列向量各个维度对应相加就得到了最终结果，这个方法的好处是
...
#### 2.2 优化方向(2)：压缩编码
　　矩阵压缩的好处不用多说，主要是能减少稀疏矩阵的存储需求，避免不必要地对零元素的操作，提高计算效率。一般对矩阵的压缩包含了以下几个方面：
- 存储格式上进行压缩
- 对元素索引进行压缩
- 元素值压缩

　　实际上要想完全记录矩阵中一个非零元的信息，一要记录它的数值，另一方面就是记录它的位置信息(索引)。压缩也就集中在压缩矩阵的索引存储空间和值的存储空间上。
...
<center>
<img src="http://wx3.sinaimg.cn/large/41f56ddcgy1fixggqza92j21h60nh78z.jpg" width="800px">
</center>

##### 2.2.2 对索引进行压缩
　　矩阵元素的通常是浮点数，但是索引是整数，因此可以对标志非零元位置的索引进行压缩，使得索引的位宽减少。已经有很多编码方法适用于这个问题，这里列举几个典型方法，具体的原理和操作方法需要很多篇幅展开讨论，只能略去了：
...
#### 2.3 优化方向(3)：负载平衡问题
　　当我们把矩阵分割和压缩以后，剩下的就是把矩阵送到片上的各个计算单元里面进行计算了。当然，我们也看到了前面的优化过程导致每个矩阵块的运算量并不能保证完全一致，这就造成了最终的SpMV处理时间是由计算最慢的计算单元所决定的。在负载极度不平衡的情况下，负载最重的单元会严重拖慢整个计算过程，所以负载平衡是不能忽视的问题。

<center>
<img src="http://wx2.sinaimg.cn/large/41f56ddcgy1fixg96rb04j210d0f13zx.jpg" width="800px">
</center>
...
#### 2.4 优化方向(4)：乘法累加器(MAC)设计
　　MAC设计也是SpMV的重点之一,但是MAC已经相对比较完善，所以在此不进行展开叙述了。
### 3. 总结
