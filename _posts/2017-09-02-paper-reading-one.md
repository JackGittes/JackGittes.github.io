...
　　但是，因为稀疏化是在训练过程中实现的,加上中间多了阈值和mask的更新过程，猜测相比“pretrain + prune + retrain”方法难免耗时要长。从头训练进行剪枝，是不是能适用到大部分现有的RNN上面，也是有些疑问。
　　
　　而且，阈值更新用到的这些超参数和更新公式，如果没有任何理论上的指导，能猜到等同于中了彩票，以后还发什么论文，直接靠买彩票为生就可以了。
